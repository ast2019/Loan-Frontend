...
import { Loader2, PlusCircle, FileText, AlertCircle, ArrowLeft } from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  // Calculate generic step for dashboard visualization only
  const getDashboardStep = () => {
    if (!loan) return 3; // Ready to start step 3
    if (loan.status === LoanStatus.WaitingForLetter) return 4;
    if (loan.status === LoanStatus.LetterIssued) return 4;
    ...
    <div className="space-y-8">
      ...
      {!loan ? (
        ...
        <div className="text-center py-12 px-4 bg-white rounded-xl border border-slate-100 shadow-sm">
          ...
          >
            شروع درخواست جدید <ArrowLeft size={18} />
          </Link>
        </div>
      ) : (
        // Active Loan Card
      ...
      {/* New Request Button when Loan Exists (Disabled per rule) */}
      {loan && (
