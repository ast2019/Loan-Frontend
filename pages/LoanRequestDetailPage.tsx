...
const LoanRequestDetailPage: React.FC = () => {
  ...
  // Check LocalStorage for download flag on mount/update (using safe wrapper logic)
  useEffect(() => {
    if (loan) {
      try {
        const downloaded = localStorage.getItem(`loan:${loan.id}:letterDownloaded`);
        if (downloaded === 'true') {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Set flag
      try {
        localStorage.setItem(`loan:${loan.id}:letterDownloaded`, 'true');
      } catch (e) {
        // ignore
      }
      setIsLetterDownloaded(true);
